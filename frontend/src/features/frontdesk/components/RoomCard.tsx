import { PiCar, PiClock, PiUsers } from 'react-icons/pi'
    ...
    >
      ...
      <div className="flex items-start justify-between gap-2">
        ...
        {!isOccupied ? (
          ...
          >
            <span className={cn('h-1.5 w-1.5 rounded-full', STATUS_BAR[room.status])} aria-hidden />
            {room.status_display}
          </span>
        ) : null}
      </div>
      ...
      {isOccupied && stay ? (
        <div className="mt-auto space-y-2 pt-3">
          <div className={cn('flex items-baseline gap-1.5', timerTone)}>
            <PiClock className="h-3.5 w-3.5 shrink-0 self-center" aria-hidden />
          ...
          <div className="flex items-center justify-between gap-2 text-2xs text-muted-foreground">
            <span className="flex items-center gap-2.5">
              <span className="inline-flex items-center gap-1">
                <PiUsers className="h-3 w-3" aria-hidden />
                {stay.occupants}
              </span>
              {stay.vehicle_plate ? (
                <span className="inline-flex items-center gap-1 font-medium text-foreground/70">
                  <PiCar className="h-3 w-3" aria-hidden />
                  {stay.vehicle_plate}
                </span>
              ) : null}
            </span>
