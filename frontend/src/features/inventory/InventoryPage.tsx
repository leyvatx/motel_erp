...
import { useExpiringLots, useLowStock, useStocks, useWarehouses } from '@/features/inventory/hooks'
          ...
          {rows.length === 0 ? (
            ...
            rows.map((lot) => (
              <TableRow key={lot.id}>
                ...
                <TableCell className="text-right tabular">{formatQuantity(lot.quantity)}</TableCell>
              </TableRow>
            ))
          )}
        </TableBody>
